...
interface WhatsAppCenterViewProps {
  ...
  onSendNotice: (notice: Partial<Notice> & { selectedStudentIds?: string[] }) => Promise<any>;
...
export default function WhatsAppCenterView({
  ...
  const handleComposeSubmit = async (e: React.FormEvent) => {
    ...
    setComposerSuccess("Message broadcast successfully dispatched! Automated delivery logs have crawled status on parent terminals.");
    setTitle("");
    setBody("");
  ...
  const filteredStudents = students.filter((s) => s.name.toLowerCase().includes(searchStudentQuery.toLowerCase()));
    ...
    <div className="space-y-8">
      ...
      <div>
        ...
        <p className="text-xs text-slate-400 mt-0.5">
          Broadcast manual notices, alerts, and holidays instantly, and monitor deep-trace automated delivery status registers.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-7 space-y-6">
            ...
            <form onSubmit={handleComposeSubmit} className="space-y-4 text-slate-800">
              ...
              <div>
                ...
                <input 
                  ...
                  className="w-full px-4 py-2 bg-slate-50 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-emerald-500 text-sm font-semibold"
                />
              </div>

              <div>
                <label className="block text-xs font-bold text-slate-500 mb-1.5 uppercase">Broadcast channel</label>
                <select
                  value={recipientType}
              ...
              {recipientType === "Selected Students" && (
                <div className="p-4 bg-slate-50 rounded-xl border space-y-3">
                  <div className="relative">
                    ...
                    <input 
                      type="text"
                      placeholder="Type student name to select..."
                      ...
                      return (
                        <div 
                          ...
                          className={`p-2 rounded-lg flex justify-between items-center cursor-pointer select-none ${
                            isChecked ? "bg-emerald-50 text-emerald-800 font-bold" : "hover:bg-slate-100"
                          }`}
                        >
                          <span>{s.name} <span className="font-mono text-[10px] text-slate-400">({s.id})</span></span>
                          <input type="checkbox" checked={isChecked} readOnly className="rounded border-slate-300 text-emerald-600 focus:ring-emerald-500 h-3.5 w-3.5" />
                        </div>
                      );
                    })}
                  </div>
              ...
              <div>
                <button 
                  type="submit"
                  className="w-full bg-emerald-600 hover:bg-emerald-700 text-white font-bold py-3 rounded-xl text-sm flex items-center justify-center gap-2 cursor-pointer transition-all shadow-md"
                >
          ...
          {/* WhatsApp Smartphone layout preview */}
          <div className="hidden sm:block bg-slate-900 text-slate-900 rounded-[36px] p-4 max-w-sm mx-auto shadow-xl relative border-[6px] border-slate-800">
            ...
            <div className="bg-[#ece5dd]/90 h-[28rem] rounded-[24px] overflow-hidden relative flex flex-col justify-between font-sans">
              ...
              <div className="p-4 flex-1 space-y-3 overflow-y-auto text-xs">
                {title || body ? (
        ...
        <div className="lg:col-span-5 space-y-4">
          ...
          <div className="bg-white p-5 rounded-2xl border border-slate-100 shadow-sm space-y-4 h-[44rem] overflow-y-auto">
